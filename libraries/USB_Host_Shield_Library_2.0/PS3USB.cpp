...
uint8_t PS3USB::Init(uint8_t parent, uint8_t port, bool lowspeed) {
        ...
        if((VID != PS3_VID && VID != PS3_ION_VID) || (PID != PS3_PID && PID != PS3NAVIGATION_PID && PID != PS3MOVE_PID && PID != PS3_ION_PID))
        ...
        if(PID == PS3_PID || PID == PS3NAVIGATION_PID || PID == PS3_ION_PID) {
                if(PID == PS3_PID || PID == PS3_ION_PID) {
#ifdef DEBUG_USB_HOST
